...
const AddressPage = () => {
  ...
  const [formData, setFormData] = useState({ title: 'Home', street: '', city: '', phone: '' });
  ...
  const handleAddAddress = async (e) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setShowForm(false);
        setFormData({ title: 'Home', street: '', city: '', phone: '' });
      }
    } catch (err) {
    ...
    <div className="address-page py-5 bg-light min-vh-100">
      <div className="container">
        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between mb-4">
          <div>
            <h1 className="fw-extrabold text-dark mb-1">Saved Delivery Addresses</h1>
            <p className="text-secondary">Manage your delivery locations for 1-click checkout.</p>
          </div>
            ...
            <form onSubmit={handleAddAddress}>
              <div className="row g-3">
                ...
                <div className="col-md-6">
                  <label className="form-label fw-semibold">City / District</label>
                  <input
                    type="text"
                    className="form-control rounded-3"
          ...
          {addresses.length === 0 ? (
            <div className="col-12">
              <div className="card border-0 rounded-4 shadow-sm bg-white p-5 text-center">
                ...
                <h5 className="fw-bold text-dark">No Saved Addresses</h5>
                <p className="text-muted">Click 'Add New Address' above to save a delivery location.</p>
              </div>
            </div>
          ) : (
            addresses.map((addr) => (
                ...
                <div className="card border-0 rounded-4 shadow-sm bg-white p-4 d-flex flex-row align-items-start justify-content-between">
                  <div className="d-flex align-items-start gap-3">
                    ...
                    <div>
                      ...
                      {addr.phone && <small className="text-secondary d-block mt-1">Phone: {addr.phone}</small>}
                    </div>
                  </div>

                  <button onClick={() => handleRemoveAddress(addr._id)} className="btn btn-link text-danger p-0" title="Delete Address">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
